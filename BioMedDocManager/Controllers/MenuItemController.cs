...
namespace BioMedDocManager.Controllers
{
    ...
    public class MenuItemController : BaseController
    {
        ...
        [NonAction]
        public async Task<IActionResult> BuildQueryMenuItem0(MenuItemQueryViewModel queryModel, CancellationToken ct)
        {
            ViewData["pageNumber"] = queryModel.PageNumber.ToString();

            // 雖然還是呼叫，主要是維持 PageSize / PageNumber 等處理
            FilterOrderBy(queryModel, TableHeaders, null);
            ...
            // ResourceKey（原本 MenuItemUrl 的查詢改成查 Resource.ResourceKey）
            if (!string.IsNullOrWhiteSpace(queryModel.ResourceKey))
            {
                var s = $"%{queryModel.ResourceKey.Trim()}%";
                q = q.Where(m => m.Resource != null && EF.Functions.Like(m.Resource.ResourceKey!, s));
            ...
            // ===== 樹狀排序：父層依自己的 DisplayOrder，子層跟在各自父層後面 =====
            // parentKey：若為根節點，用自己 MenuItemId；若為子節點，用父節點 MenuItemParentId
            // 這樣就能做到：
            //   parent1 (parentKey=parent1Id, isRoot=0)
            //   child1-1 (parentKey=parent1Id, isRoot=1)
            //   child1-2 (parentKey=parent1Id, isRoot=1)
            //   parent2 ...
            ...
            var result = BuildRows(
                ...
                keyMode: KeyMode.PropertyName,
                includeRowNum: true,
                payloadProps: new[] { "MenuItemId", "MenuItemParentId" } // 讓 Index 可以判斷 root / child
            );
